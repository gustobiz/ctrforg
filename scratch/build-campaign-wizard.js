...
interface FollowupRuleInput {
  ...
  sendTimeTz?: string; // e.g. "Asia/Kolkata"
...
/** Helpers: Convert 24-hour format string ("09:00", "17:00") <-> 12-hour components */
...
export default function NewCampaignWizard() {
  ...
  // Form states (Canonical single source of truth for Main Email)
  ...
  // Followup states (with independent send-time per step)
  ...
  // Follow-up Template Select & Reset Handlers
  ...
  const handleAddFollowup = () => {
    ...
    setFollowupRules(prev => [
      ...prev,
      {
        delayDays: prev.length === 0 ? 3 : 4,
        sendTime: '10:00',
        sendTimeTz: sendWindowTz || getBrowserTimezone(),
        ruleType: 'not_opened',
  ...
  const currentReviewLead = leads.find(l => l.id === selectedLeadIds[reviewLeadIndex]) || leads[0];
  const creatorName = currentReviewLead?.creator_name || 'Creator';
  const rawDataForReview = currentReviewLead?.ai_analysis?.raw_data || {};
  const reviewVars: Record<string, string> = currentReviewLead ? {
    ...rawDataForReview,
    name: creatorName,
    creator_name: creatorName,
    first_name: creatorName ? creatorName.split(' ')[0] : 'Creator',
    last_name: creatorName ? creatorName.split(' ').slice(1).join(' ') : '',
    email: getLeadEmail(currentReviewLead) || 'creator@youtube.com',
    channel_name: currentReviewLead.channel_name || creatorName,
    subscriber_count: String(currentReviewLead.subscriber_count || '150K'),
    video_title: getLeadVideoTitle(currentReviewLead) || '10 Secrets to Scale Your YouTube CTR',
    niche: currentReviewLead.ai_analysis?.creator_niche || 'YouTube Growth',
  } : {
    name: 'Jane Doe',
    ...
    video_title: '10 Secrets to Scale Your YouTube CTR',
    niche: 'Tech',
  };
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#09090b] text-[#f4f4f5] antialiased">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-6 py-8">
        ...
        {/* Step 2: MAIN EMAIL (Bug 1 Fix — Single Canonical State & Full Interactive Editor) */}
        {currentStep === 2 && (
          <div className="space-y-6 animate-in fade-in-50 duration-150">
            <div className="space-y-1">
              <h2 className="text-xl font-bold text-zinc-100 flex items-center gap-2">
                <Mail className="h-5 w-5 text-emerald-400" /> Main Email
              </h2>
              <p className="text-xs text-zinc-500 uppercase tracking-wider">
                Draft and customize the primary cold outreach email. Personalize with dynamic variables and preview in real-time.
              </p>
            </div>

            <div className="grid lg:grid-cols-12 gap-8 items-start">
              {/* Left Column (7 cols): Template Picker, Subject, Body Editor, Dynamic Variables */}
              <div className="lg:col-span-7 space-y-5">
                ...
                <div className="p-4 border border-white/[0.04] bg-zinc-900/10 rounded-2xl space-y-3">
                  ...
                  {loadingTemplates ? (
                    ...
                    <select
                      ...
                      className="w-full bg-zinc-950 border border-white/[0.06] rounded-xl px-3.5 py-2.5 text-xs text-zinc-200 focus:outline-none focus:border-emerald-500 cursor-pointer"
                    >
                ...
                {/* Subject Line Input */}
                <div className="p-4 border border-white/[0.04] bg-zinc-900/10 rounded-2xl space-y-2">
                  ...
                  <input
                    ...
                    placeholder="Quick thought on {{video_title}}"
                    className="w-full bg-zinc-950 border border-white/[0.06] rounded-xl px-3.5 py-2.5 text-xs text-zinc-150 font-bold focus:outline-none focus:border-emerald-500 transition-colors"
                  />
                </div>
                ...
                {/* Dynamic Variables Board */}
                <div className="p-4 border border-white/[0.04] bg-zinc-900/10 rounded-2xl space-y-2.5">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {[
                      ...
                      { tag: 'creator_name', label: 'Full Name' },
                      { tag: 'channel_name', label: 'Channel Name' },
                      { tag: 'video_title', label: 'Latest Video' },
                      { tag: 'subscriber_count', label: 'Subscriber Count' },
                      { tag: 'niche', label: 'Creator Niche' },
                      { tag: 'email', label: 'Contact Email' },
                      ...(sheetHeaders || []).filter(h => !['name', 'email', 'channel', 'subs'].some(k => h.toLowerCase().includes(k))).map(h => ({ tag: h, label: h }))
                    ].map(({ tag, label }) => (
                      <button
                        key={tag}
                        type="button"
              ...
              <div className="lg:col-span-5 space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
                    <Eye className="h-4 w-4 text-emerald-400" /> LIVE CLIENT PREVIEW
                  </span>
                  <span className="text-[10px] text-emerald-400 font-extrabold uppercase bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full">
                    Real-time
                  </span>
                </div>

                <div className="border border-white/[0.04] rounded-2xl overflow-hidden bg-zinc-950 shadow-2xl">
                  <div className="p-4 border-b border-white/[0.04] space-y-2 bg-zinc-900/40">
                    <div className="flex items-baseline text-xs">
                      <span className="text-zinc-500 w-16 font-bold uppercase">To:</span>
                      <span className="text-zinc-300 font-semibold">{currentReviewLead ? (getLeadEmail(currentReviewLead) || currentReviewLead.creator_name) : 'creator@youtube.com'}</span>
                    </div>
                    <div className="flex items-baseline text-xs border-t border-white/[0.02] pt-2">
                      <span className="text-zinc-500 w-16 font-bold uppercase">Subject:</span>
                      <span className="text-zinc-100 font-bold">
                        {interpolateVariablesForClient(subjectOverride || selectedTemplateObj?.subject || '(No Subject)', reviewVars)}
                      </span>
                    </div>
                  </div>

                  <div className="p-5 min-h-[240px] max-h-[420px] overflow-y-auto bg-[#0c0c0e]">
                    <div
                      className="text-xs text-zinc-200 space-y-3 font-sans leading-relaxed prose prose-invert max-w-none"
                      dangerouslySetInnerHTML={{
                        __html: interpolateVariablesForClient(htmlBodyOverride || selectedTemplateObj?.html_body || '<p class="text-zinc-500 italic">No Body Content</p>', reviewVars)
                      }}
                    />
                    {globalSig?.is_enabled && globalSigHtml && (
                      <div 
                        className="mt-6 pt-4 border-t border-white/10 text-xs text-zinc-200 [&_.sig-name]:text-zinc-100 [&_.sig-role]:text-zinc-400 [&_a]:text-emerald-400 [&_a:hover]:underline"
                ...
                {/* Email Signature Status Banner */}
                <div className="p-3.5 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.02] flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
                    <span className="font-bold text-zinc-300">Email Signature: Active</span>
                  </div>
                  <Link href="/settings" target="_blank" className="text-[10px] text-emerald-400 hover:underline font-bold uppercase">
                    Configure Signature ↗
                  </Link>
                </div>
              </div>
        ...
        {/* Step 3: Configure Follow-up Sequence & Independent Send Time Configuration (Bug 6) */}
        {currentStep === 3 && (
          <div className="space-y-6 animate-in fade-in-50 duration-150">
            <div className="space-y-1">
              <h2 className="text-xl font-bold text-zinc-100">Setup Follow-up Automation Rules</h2>
              <p className="text-xs text-zinc-500 uppercase tracking-wider">
                Configure independent send-time, delay, triggers, and templates for each follow-up sequence step.
              </p>
            </div>
                ...
                return (
                  <div key={idx} className="p-5 border border-white/[0.04] bg-zinc-900/10 rounded-2xl relative space-y-4 animate-in slide-in-from-top-4 duration-150">
                    ...
                    <h3 className="font-extrabold text-xs uppercase tracking-wider text-emerald-400 flex items-center">
                      <Target className="h-3.5 w-3.5 mr-1.5" /> FOLLOW-UP #{idx + 1} SEQUENCE CONFIGURATION
                    </h3>

                    {/* Step Rule Controls with Independent Send Time */}
                    <div className="grid md:grid-cols-4 gap-4 text-xs">
                      {/* Delay in Days */}
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Delay After Previous</label>
                      ...
                      {/* Independent Send Time Picker */}
                      <TimePicker12Hour 
                        label="Send Time"
                        value24={rule.sendTime || '10:00'}
                        onChange24={(newTime) => handleUpdateFollowup(idx, 'sendTime', newTime)}
                      />
                    ...
                    <div className="border border-white/[0.04] rounded-xl p-4 bg-zinc-950/60 space-y-3 mt-4">
                      <div className="flex justify-between items-center pb-2 border-b border-white/[0.04]">
                        <div className="flex items-center gap-3">
                          ...
                          <select
                            ...
                            className="bg-zinc-900 border border-white/[0.06] rounded-lg px-2.5 py-1 text-xs text-zinc-200 cursor-pointer"
                          >
                            <option value="">-- Custom Override / AI Generation --</option>
                            {templates.map(t => (
                      ...
                      <div className="space-y-1">
                        ...
                        <input 
                          ...
                          placeholder={selectedRuleTemplate?.subject || 'Quick thought on "{{video_title}}"'}
        ...
        {/* Step 4: Review & Verify (Bug 8 Fix — Full Accurate Sequence & Delivery Representation) */}
        {currentStep === 4 && (
          <div className="space-y-6 animate-in fade-in-50 duration-150">
            <div className="space-y-1">
              <h2 className="text-xl font-bold text-zinc-100">Review & Verify</h2>
              <p className="text-xs text-zinc-500 uppercase tracking-wider">
                Review the exact email sequences, calculated send times, dynamic variable merges, and delivery schedule before launching.
              </p>
            </div>
            ...
            {/* Sequence Step Navigation Tabs (Main Email + Follow-up #1, #2...) */}
            ...
            {/* Sequence & Delivery Detailed Representation Cards */}
            <div className="grid md:grid-cols-2 gap-4">
              
              {/* Follow-up Sequence Calculated Details */}
              <div className="p-5 rounded-2xl border border-white/[0.04] bg-zinc-900/10 space-y-3">
                <h4 className="text-xs font-bold text-zinc-300 uppercase tracking-wider flex items-center gap-2">
                  <Calendar className="h-4 w-4 text-emerald-400" /> Sequence Timing Breakdown
                </h4>

                <div className="space-y-2.5 text-xs">
                  <div className="p-3 bg-zinc-950/60 rounded-xl border border-white/[0.04] space-y-1">
                    <span className="text-[10px] font-bold text-emerald-400 uppercase block">Step 1: Main Email</span>
                    <p className="text-zinc-300 font-semibold">
                      Dispatched on campaign start &bull; Next Eligible: {formattedNextEligibleSend}
                    </p>
                  </div>

                  {followupRules.map((rule, idx) => {
                    // Compute calculated follow-up execution date starting from nextEligibleSendDate
                    const calculatedSendDate = calculateFollowupScheduledTime({
                      previousStepSentAt: nextEligibleSendDate,
                      delayDays: rule.delayDays,
                      sendTime: rule.sendTime || '10:00',
                      sendTimeTz: rule.sendTimeTz || sendWindowTz,
                      campaignWindowConfig: sendingWindowConfig,
                    });
                    const formattedCalculatedDate = formatDateTimeInTimezone(calculatedSendDate, rule.sendTimeTz || sendWindowTz);

                    return (
                      <div key={idx} className="p-3 bg-zinc-950/60 rounded-xl border border-white/[0.04] space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-bold text-emerald-400 uppercase">
                            Step {idx + 2}: Follow-up #{idx + 1}
                          </span>
                          <span className="text-[10px] text-zinc-400 font-mono">
                            {rule.delayDays} day(s) after Step {idx + 1} at {formatTo12HourDisplay(rule.sendTime || '10:00')}
                          </span>
                        </div>
                        <p className="text-zinc-200 font-bold">
                          Calculated Send: {formattedCalculatedDate}
                        </p>
                        <p className="text-[10px] text-zinc-500">
                          Condition: {rule.ruleType.replace(/_/g, ' ')} &bull; Thread: {rule.threadMode || 'reply'}
                        </p>
                      </div>
                    );
                  })}
              ...
              <div className="p-5 rounded-2xl border border-white/[0.04] bg-zinc-900/10 space-y-3">
                <h4 className="text-xs font-bold text-zinc-300 uppercase tracking-wider flex items-center gap-2">
                  <Sliders className="h-4 w-4 text-emerald-400" /> Delivery Parameters
                </h4>

                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="p-2.5 bg-zinc-950/60 rounded-xl border border-white/[0.04]">
                    <span className="text-[10px] text-zinc-500 font-bold uppercase block">Launch Mode</span>
                    <span className="font-extrabold text-zinc-200">
                      {scheduleMode === 'immediate' ? 'Run Immediately' : 'Scheduled'}
                    </span>
                  </div>
                  ...
                  <div className="p-2.5 bg-zinc-950/60 rounded-xl border border-white/[0.04]">
                    <span className="text-[10px] text-zinc-500 font-bold uppercase block">Allowed Days</span>
                    <span className="font-extrabold text-zinc-200">
                      {sendWindowDays.map(d => DAYS_OF_WEEK.find(x => x.id === d)?.label).join(', ')}
                    </span>
                  </div>

                  <div className="p-2.5 bg-zinc-950/60 rounded-xl border border-white/[0.04]">
                    <span className="text-[10px] text-zinc-500 font-bold uppercase block">Sending Speed</span>
                  ...
                  <div className="p-2.5 bg-zinc-950/60 rounded-xl border border-white/[0.04]">
                    <span className="text-[10px] text-zinc-500 font-bold uppercase block">Delays</span>
                    <span className="font-extrabold text-zinc-200">
                      {delayMinVal}{delayMinUnit[0]} - {delayMaxVal}{delayMaxUnit[0]}
                    </span>
                  </div>
                </div>

                <div className="p-3 bg-emerald-500/[0.04] border border-emerald-500/20 rounded-xl text-xs space-y-1">
                  <span className="text-[10px] font-bold uppercase text-emerald-400 block">Next Eligible Send Window</span>
                  <p className="font-extrabold text-zinc-150">
                    {formattedNextEligibleSend} ({sendWindowTz})
                  </p>
                </div>
              </div>
        ...
        {/* Step 5: Delivery Configuration & Live Window UX (Bug 2, 4, 5 Fix) */}
        {currentStep === 5 && (
          <div className="space-y-6 animate-in fade-in-50 duration-150">
            <div className="space-y-1">
            ...
            <div className="border border-white/[0.04] rounded-3xl p-6 bg-zinc-900/10 space-y-6">
              ...
              {/* Launch Mode Toggle (Run Immediately vs Schedule Campaign) */}
              <div className="space-y-3 border-b border-white/[0.04] pb-5">
                <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider block">Campaign Launch Mode</label>
              ...
              <div className="border border-white/[0.06] bg-zinc-950/60 p-5 rounded-2xl space-y-4">
                <h4 className="text-xs font-extrabold text-zinc-200 uppercase tracking-wider flex items-center gap-2">
                  <ShieldCheck className="h-4 w-4 text-emerald-400" /> Campaign Pre-Launch Summary
                </h4>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 text-xs">
                  ...
                  <div className="p-3 bg-zinc-900/40 rounded-xl border border-white/[0.04] space-y-1">
                    ...
                    <p className="text-sm font-extrabold text-zinc-150 truncate">{selectedTemplateObj?.name || 'Custom Main Email'}</p>
